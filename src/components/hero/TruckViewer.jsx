...
export default function TruckViewer({ poster = FALLBACK_IMG, autoRotate = true }) {
  ...
  useEffect(() => {
    let cancelled = false;
    // Mobile now mounts the real 3D vehicle too. The old build bailed out here
    // because model-viewer swallows touch and traps page scroll — that is solved
    // by touch-action="pan-y" below (vertical drag scrolls the page, horizontal
    // drag orbits the truck), so the viewer no longer has to be withheld on phones.
